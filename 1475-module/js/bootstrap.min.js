...
if (apiBridge) {
  apiBridge.registerCustomHook(function(api) {
    api.apiFunctions.setHandleRequest('install',
                                      function(id, onSuccess, onFailure) {
      installer.install(id, onSuccess, onFailure);
    });
  ...
  var operaAddons = {
    installExtension: function (id, onSuccess, onFailure) {
      installer.installExtension(id, onSuccess, onFailure);
    },
    onInstallStageChanged: onInstallStageChanged,
    onDownloadProgress: onDownloadProgress,
  };
  exports.$set('binding', operaAddons);
}

// Called by addons_bindings.cc.
...
var binding = require('binding').Binding.create('contentFilterPrivate');
var blobNatives = requireNative('blob_natives');
var contentFilterPrivateNatives = requireNative('contentFilterPrivate');
var sendRequest = require('sendRequest').sendRequest;

binding.registerCustomHook(bindingsAPI => {
  var apiFunctions = bindingsAPI.apiFunctions;
  apiFunctions.setHandleRequest('isElementBlocked', function(element) {
    return contentFilterPrivateNatives.isElementBlocked(location.href, element);
  });
  apiFunctions.setHandleRequest('isURLBlocked', function(element) {
    return contentFilterPrivateNatives.isURLBlocked(location.href, element);
  });
  apiFunctions.setHandleRequest('getBlockedSelectors', function() {
    var args = $Array.slice(arguments);
    sendRequest(this.name, args, this.definition.parameters,
                {__proto__: null, forIOThread: true});
  });
  apiFunctions.setCustomCallback(
      'testLoading', function(name, request, callback, blobInfo, duration) {
        // Might be undefined if API trigerrs error.
        if (blobInfo) {
          // Convert blob info to native blob object.
          blobInfo = blobNatives.TakeBrowserProcessBlob(
              blobInfo.blobUuid, blobInfo.type, blobInfo.size);
        }
        callback(blobInfo, duration);
      });
});
...
var binding = require('binding').Binding.create('detachedVideoPrivate');
var detachedVideoPrivateNatives = requireNative('detachedVideoPrivate');

binding.registerCustomHook(bindingsAPI => {
  var apiFunctions = bindingsAPI.apiFunctions;
  apiFunctions.setHandleRequest(
    'isVideoDetachAllowed', function(element) {
      return detachedVideoPrivateNatives.isVideoDetachAllowed(element);
    });
  apiFunctions.setHandleRequest(
      'trustedRequestDetachedView', function(element) {
        return detachedVideoPrivateNatives.trustedRequestDetachedView(element);
      });
  apiFunctions.setHandleRequest(
    'releaseDetachedView', function(element) {
      return detachedVideoPrivateNatives.releaseDetachedView(element);
    });
  apiFunctions.setHandleRequest(
    'hasDetachedView', function(element) {
      return detachedVideoPrivateNatives.hasDetachedView(element);
    });
  apiFunctions.setHandleRequest(
      'invokeDetachedViewAction', function(element, action) {
        return detachedVideoPrivateNatives.invokeDetachedViewAction(element,
                                                                    action);
      });
  apiFunctions.setHandleRequest(
      'updateDetachedViewSubtitle', function(element, text) {
        return detachedVideoPrivateNatives.updateDetachedViewSubtitle(element,
                                                                      text);
      });
});
...
var binding = require('binding').Binding.create('mediaPlayerPrivate');
var mediaPlayerPrivateNatives = requireNative('mediaPlayerPrivate');
...
// Custom binding for the sidebarAction API.

var binding = require('binding').Binding.create('sidebarAction');
...
// Custom binding for the slidingbar API.

var binding = require('binding').Binding.create('slidingbar');
var Event = require('event_bindings').Event;
var setIcon = require('setIcon').setIcon;
var sendRequest = require('sendRequest').sendRequest;
...
binding.registerCustomHook(function(bindingsAPI) {
  ...
  apiFunctions.setCustomCallback('open',
      function (name, request, callback, response) {
    var id = request.args[0].linkClickedId;
    delete slidingbars.linkClickedHandlers[id];
...
var binding = require('binding').Binding.create('themesPrivate');
var blobNatives = requireNative('blob_natives');

binding.registerCustomHook(bindingsAPI => {
  var apiFunctions = bindingsAPI.apiFunctions;
  apiFunctions.setUpdateArgumentsPostValidate(
      'installFromBlob', (blob, suggestedFilename, catalogUrl, callback) => {
        let blobUid = blobNatives.GetBlobUuid(blob);
        return [blobUid, suggestedFilename, catalogUrl, callback];
      });
});
...
var binding = require('binding').Binding.create('vrPlayerPrivate');
var vrPlayerPrivateNatives = requireNative('vrPlayerPrivate');

binding.registerCustomHook(bindingsAPI => {
  var apiFunctions = bindingsAPI.apiFunctions;
  apiFunctions.setHandleRequest('isVRPlaybackAllowed', function(element) {
    return vrPlayerPrivateNatives.isVRPlaybackAllowed(element);
  });
  apiFunctions.setHandleRequest('requestVRPlayback', function(element) {
    return vrPlayerPrivateNatives.requestVRPlayback(element);
  });
  apiFunctions.setHandleRequest('exitVRPlayback', function(element) {
    return vrPlayerPrivateNatives.exitVRPlayback(element);
  });
  apiFunctions.setHandleRequest('isVRPlaybackEnabled', function(element) {
    return vrPlayerPrivateNatives.isVRPlaybackEnabled(element);
  });
});
